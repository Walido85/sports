...
def get_utc_iso(date_str: str, time_str: str, server_tz: str) -> str:
    """Takes the scraped visual time, assigns it to the exact server timezone, and computes absolute UTC."""
    ...
    try:
        ...
        # 1. Attach the exact server timezone extracted from the webpage (e.g. "America/Denver")
        try:
            dt_aware = dt_naive.replace(tzinfo=ZoneInfo(server_tz))
        except:
            dt_aware = dt_naive.replace(tzinfo=ZoneInfo("America/Denver")) # Fallback safety
            
        # 2. Mathmatically convert it to pure UTC
...
async def deep_scrape_match_details(page, match_url: str) -> dict:
    ...
    try:
        ...
        await new_page.goto(match_url, wait_until="networkidle", timeout=30000)
...
async def scrape_fixtures(page, league: dict) -> None:
    ...
    # 🌍 Read the timezone forced by YSScores server
    ...
    for el in elements:
        ...
        try:
            ...
            # Use the server_tz we detected above to calculate proper UTC math
